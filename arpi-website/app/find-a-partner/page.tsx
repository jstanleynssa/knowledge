...
export default function PartnersPage() {
  ...
  useEffect(() => { setPage(1) }, [name, stateFilter, selectedRoles, directionFilter, origin, radius])
          ...
          <div className="container">
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '48px', flexWrap: 'wrap' }}>
              <div style={{ position: 'relative', zIndex: 1, flexShrink: 0 }}>
                ...
                <h1 style={{ marginBottom: 0 }}>Find a<br />CELP® Partner</h1>
              </div>
            ...
            {!loading && !loadError && partners.length > 0 && (
              <>
                ...
                <div>
                  ...
                  {totalPages > 1 && (
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '16px', marginTop: '32px' }}>
                      <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                        style={{ padding: '8px 18px', borderRadius: '8px', border: '1px solid #e5e7eb', background: page === 1 ? '#f3f4f6' : '#fff', color: page === 1 ? '#9ca3af' : '#1f2937', cursor: page === 1 ? 'default' : 'pointer', fontWeight: 600, fontSize: '0.875rem' }}>
                        ← Previous
                      </button>
                      <span style={{ fontSize: '0.875rem', color: '#6b7280' }}>Page {page} of {totalPages}</span>
                      <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                        style={{ padding: '8px 18px', borderRadius: '8px', border: '1px solid #e5e7eb', background: page === totalPages ? '#f3f4f6' : '#fff', color: page === totalPages ? '#9ca3af' : '#1f2937', cursor: page === totalPages ? 'default' : 'pointer', fontWeight: 600, fontSize: '0.875rem' }}>
                        Next →
                      </button>
                    </div>
                  )}
                  ...
                  {/* ── Join CTA ── */}
                  <div style={{
                    marginTop: '40px', padding: '20px 28px',
                    background: '#f0faf5', borderRadius: '12px',
                    display: 'flex', alignItems: 'center',
                    justifyContent: 'space-between', gap: '24px', flexWrap: 'wrap',
                  }}>
                    <p style={{ margin: 0, fontSize: '0.9375rem', color: '#1f2937' }}>
                      <strong>Not listed?</strong> Join the CELP® Referral Network and start receiving referrals from CELP® professionals in your market.
                    </p>
                    <a href="/partners/apply" style={{
                      display: 'inline-block', padding: '10px 22px', borderRadius: '8px',
                      background: '#2a6b54', color: '#fff', fontWeight: 700,
