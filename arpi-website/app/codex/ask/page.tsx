...
function FeedbackBar({ turn, onFeedback, onRewriteFeedback }: {
  ...
  const BD = 'var(--blue-dark)'; const BM = 'var(--blue-400)'; const BL = '#e1ddd2';
...
function RewriteFeedbackBar({ onFeedback }: { onFeedback: (type: 'approve' | 'correct' | 'reject', note?: string) => void }) {
  const [mode, setMode] = useState<'idle' | 'correcting'>('idle');
  const [note, setNote] = useState('');
  const BD = 'var(--blue-dark)'; const BC = '#cdc4ad'; const BR = '#4a3c20'; const RM = '#b02a34';
...
.answer-body h3, .answer-body h4 { margin: 16px 0 6px; font-size: 1em; font-weight: 700; color: var(--green-dark); }
      ...
      <header style={{ borderBottom: `1px solid ${RULE}`, padding: '14px 0', flexShrink: 0, position: 'sticky', top: 0, background: PAPER, zIndex: 10 }}>
        <div style={{ maxWidth: 760, margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
            ...
            <a href="https://www.arpinstitute.com" style={{ fontSize: 13, color: SOFT, textDecoration: 'none' }}>arpinstitute.com &rsaquo;</a>
          </div>
        </div>
      </header>
        ...
        <footer style={{ borderTop: `1px solid ${RULE}`, padding: '28px 24px 48px', maxWidth: 760, margin: '0 auto', width: '100%' }}>
          <div style={{ color: SOFT, fontSize: 12, lineHeight: 1.7 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 16px', marginBottom: 8 }}>
              <a href="https://arpinstitute.com/credentials/nssa" target="_blank" rel="noopener" style={{ color: NAVY, textDecoration: 'none', fontWeight: 600 }}>Social Security Certification &rsaquo;</a>
              <a href="https://arpinstitute.com/credentials/irmaacp" target="_blank" rel="noopener" style={{ color: NAVY, textDecoration: 'none', fontWeight: 600 }}>IRMAA Certification &rsaquo;</a>
              <a href="https://www.arpinstitute.com/directory" target="_blank" rel="noopener" style={{ color: NAVY, textDecoration: 'none', fontWeight: 600 }}>Find an Advisor &rsaquo;</a>
            </div>
            <div style={{ color: '#cdc4ad', fontSize: 11, borderTop: `1px solid ${RULE}`, paddingTop: 8 }}>
              ARPI Knowledge Base provides educational reference material based on SSA POMS. Not individualized legal, financial, or benefits advice. Verify current rules with SSA before making filing decisions.
            </div>
          </div>
        </footer>
      )}
