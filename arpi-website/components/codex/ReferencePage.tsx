...
import type { ReferencePage, BodySection, FaqItem, PrimarySource, ReferenceComponent } from '@/lib/codex-types';
import { SectionFeedback } from '@/components/codex/LazyFeedback';
...
.cite{
  ...
  background:var(--cite-bg);border:1px solid #e5e7eb;border-radius:3px;
  padding:12px 14px;margin:14px 0;color:#374151;
...
.cite code{font-family:ui-monospace,"SF Mono",Menlo,monospace;font-size:13px;
  background:#fff;padding:1px 5px;border-radius:2px;border:1px solid #e5e7eb;
  white-space:normal;overflow-wrap:break-word;word-break:break-word}
.cite a{color:#374151;font-weight:600}
...
.example{background:#f3f4f6;border-radius:2px;padding:20px 22px;margin:24px 0}
...
.cta{background:#0D3B5C;color:#fff;border-radius:4px;padding:28px 28px 30px;margin:44px 0 8px}
.cta-irmaa{background:var(--red-dark)}
...
.cta-btn{display:inline-block;font-family:ui-sans-serif,system-ui;font-size:15px;font-weight:600;
  background:#fff;color:#0D3B5C;text-decoration:none;padding:13px 22px;border-radius:3px;
  transition:transform .15s ease,box-shadow .15s ease}
.cta-irmaa .cta-btn{color:var(--red-dark)}
...
.foot-sep{color:#d1d5db}
.foot-links{display:flex;flex-wrap:wrap;gap:4px 16px;margin-bottom:10px}
.foot-disc{font-size:11.5px;color:#6b7280;border-top:1px solid var(--rule);padding-top:10px;line-height:1.6}
        ...
        <div key={i} className="faq-item" style={{ position: 'relative', opacity: rewritingFaq?.[i] ? 0.6 : 1, transition: 'opacity .2s' }}>
          ...
          {item.ref_page_id && item.ref_page_slug && item.ref_page_category && (
            <p style={{ marginTop: 6, fontSize: 14 }}>
              🔗 <a href={`/codex/${item.ref_page_category}/${item.ref_page_slug}`} style={{ color: 'var(--green-mid)', fontWeight: 600 }}>{item.ref_page_title}</a>
            </p>
          )}
          {onFaqFeedback && (
            <SectionFeedback
...
/* Prevent serif font from bleeding into the ARPI site nav */
body{font-family:var(--font-inter),-apple-system,BlinkMacSystemFont,sans-serif;font-size:16px;line-height:1.6}
.kb-embed-root{
...
export function ReferencePageComponent({ page, components, previewMode, embedded, onSectionFeedback, onFaqFeedback, sectionLearned, rewritingSection, sectionExistingFeedback, faqLearned, rewritingFaq, faqExistingFeedback }: ReferencePageProps) {
  ...
  const ctaUrl = `/find-an-advisor?utm_source=knowledge&utm_medium=referral&utm_campaign=kb_cta&utm_content=${page.slug}`;
      ...
      <div className="kb-embed-root">
        ...
        <div className="wrap">
          <nav className="crumbs" aria-label="Breadcrumb">
            <a href="/codex">Knowledge Base</a>
            <span className="sep">/</span>
            <a href={`/codex${categoryPath}`}>{categoryLabel}</a>
            {page.eyebrow && (
              <><span className="sep">/</span><a href={`/codex${categoryPath}?topic=${encodeURIComponent(page.eyebrow)}`}>{page.eyebrow}</a></>
            )}
            <span className="sep">/</span>
            {page.title}
          </nav>
          ...
          <div className="review">
            ...
            This page is part of the ARPI Knowledge Base and is reviewed on a quarterly cycle for accuracy against current SSA guidance.
            <div className="foot-disc" style={{marginTop:'12px',paddingTop:'12px',paddingBottom:'32px',borderTop:'1px solid var(--rule)'}}>
  ...
  return (
    <html lang="en">
      <head>
        ...
        <meta property="og:site_name" content="ARPI Knowledge Base" />
      ...
      <body>
        ...
        <header style={{borderBottom:'1px solid #e5e7eb',background:'#fff',position:'sticky',top:0,zIndex:50}}>
          <div style={{maxWidth:1152,margin:'0 auto',padding:'12px 24px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:16}}>
            <a href="https://www.arpinstitute.com" style={{flexShrink:0}}>
              <img
            ...
            <nav style={{display:'flex',alignItems:'center',gap:20,flexWrap:'wrap'}}>
              {[
                ['About Us','https://www.arpinstitute.com/about'],
                ...
                ['Find an Advisor','https://www.arpinstitute.com/directory'],
                ['Contact Us','https://www.arpinstitute.com/contact'],
                ['Log In','https://www.arpinstitute.com/login'],
              ].map(([label,href]) => (
