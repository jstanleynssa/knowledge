...
async function fetchDirectoryMembers() {
  ...
  // Inclusion gate: must be active, have real bio content, not opted out, not admin-excluded.
  return all.filter(m => {
  ...
  return (
    <div style={{ background: GRAY.bg, borderRadius: '10px', padding: '2rem' }}>
      ...
      <button
        ...
        disabled={sending}
        style={{
          background: '#139a4d', color: 'white', border: 'none', borderRadius: '3px',
          padding: '13px 28px', fontSize: '0.9rem', fontWeight: 600, letterSpacing: '0.03em',
...
export default function AdvisorProfile({ member, slug }) {
  ...
  // Stable per-advisor seed for deterministic content variation.
  const seed = member.email || slug

  // H1 — clean, SEO-relevant, not spammy.
  // Format: "{Name}, {Credential(s)} Advisor · {City}, {State}"
  // Ignores the directory_h1 DB override (those were keyword-stuffed).
  const certLabel = hasNssa && hasIrmaa
    ? 'NSSA® & IRMAACP™ Advisor'
    : hasNssa
      ? 'NSSA® Advisor'
      : 'IRMAACP™ Advisor'
  const cityState = [member.city, stateToken(member.state).toUpperCase()].filter(Boolean).join(', ')
  const h1Auto = cityState ? `${name}, ${certLabel} · ${cityState}` : `${name}, ${certLabel}`
  const h1 = (member.directory_h1 || '').trim() || h1Auto
  ...
  const metaDesc = paragraphs[0]
    ...
    : truncateAtWord(`${name} is an ${certLabel}${cityState ? ` based in ${cityState}` : ''}, helping clients with Social Security and Medicare planning.`, 155)
  ...
  const linkStyle = { color: '#139a4d', textDecoration: 'underline' }
  const nssaLink = <a key="nl" href={NSSA_COURSE} style={linkStyle}>{nssaAnchor}</a>
  const irmaaLink = <a key="il" href={IRMAA_COURSE} style={{ ...linkStyle, color: '#9d174d' }}>{irmaaAnchor}</a>
  ...
  const pillBtn = {
    display: 'block', textAlign: 'center', padding: '13px 24px', marginBottom: '10px',
    border: '2px solid #276f46', borderRadius: '3px', background: 'transparent',
    color: '#276f46', fontWeight: 600, fontSize: '0.9rem', textDecoration: 'none',
    letterSpacing: '0.03em', transition: 'background 0.2s, color 0.2s',
  }
  const pillBtnPrimary = {
    ...pillBtn, background: '#139a4d', color: '#fff', border: '2px solid transparent',
  }
      ...
      <div style={{ color: GRAY.dark }}>
        ...
        <section className="section-pad" style={{ background: GRAY.bg, padding: '3rem 2rem' }}>
          <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            <h1 className="page-h1" style={{ fontSize: '2.4rem', fontWeight: 700, color: '#276f46', marginBottom: '2rem' }}>{h1}</h1>
            <div className="hero-grid" style={{ display: 'grid', gridTemplateColumns: '280px 1fr 280px', gap: '2.5rem', alignItems: 'start' }}>
              ...
              <div>
                ...
                {(member.job_title || member.company) && (
                  <p style={{ fontFamily: 'Inter, system-ui, sans-serif', fontWeight: 400, fontSize: '16px', color: GRAY.dark, marginTop: 0, marginBottom: '18px' }}>
                ...
                {phone && <p style={{ fontSize: '16px', marginTop: 0, marginBottom: '4px' }}><a href={`tel:${phone.replace(/[^0-9+]/g, '')}`} style={{ color: '#139a4d', textDecoration: 'none' }}>{phone}</a></p>}
                {web && <p style={{ fontSize: '15px', marginTop: 0, marginBottom: '16px' }}><a href={web} target="_blank" rel="nofollow noopener noreferrer" style={{ color: '#139a4d', textDecoration: 'none' }}>{cleanWebsite(member.website)}</a></p>}
              ...
              <div className="actions-col">
                {web && <a href={web} target="_blank" rel="nofollow noopener noreferrer" style={pillBtnPrimary}>Visit Website</a>}
          ...
          <div className="body-grid" style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 420px', gap: '3rem', alignItems: 'start' }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '1.5rem' }}>
                <span style={{ display: 'inline-block', width: '5px', height: '34px', background: '#139a4d', borderRadius: '2px' }} />
                ...
                <ol style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '10px', fontSize: '14px', color: GRAY.text }}>
                  {breadcrumbs.map((b, i) => (
                    <li key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: '10px' }}>
                      {b.href
                        ? <a href={b.href} style={{ color: '#139a4d', textDecoration: 'none', fontWeight: 600 }}>{b.label}</a>
        ...
        {/* Value-prop band — ARPI style */}
        <section className="section-pad" style={{ background: '#f8f9fa', padding: '5rem 2rem', borderTop: '1px solid #e5e7eb' }}>
          <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
            <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
              <div style={{ fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#139a4d', marginBottom: 12 }}>Why ARPI-Certified</div>
              <h2 style={{ fontFamily: 'var(--font-merriweather), Georgia, serif', fontSize: 'clamp(1.6rem, 2.5vw, 2.2rem)', fontWeight: 700, color: '#111827', lineHeight: 1.25, letterSpacing: '-0.01em' }}>How an ARPI-Certified Advisor Can Help You</h2>
              <p style={{ fontSize: '1rem', color: '#6b7280', maxWidth: '640px', margin: '16px auto 0', lineHeight: 1.75 }}>
            ...
            <div className="valueprop-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2px', background: '#e5e7eb', border: '1px solid #e5e7eb', borderRadius: '6px', overflow: 'hidden' }}>
              {[
                ['/directory/social-security-mistakes.png', 'Avoid Costly Claiming Mistakes', 'Social Security rules are complex — small missteps can cost thousands in lost benefits. An ARPI-certified advisor ensures you make informed decisions and avoid costly errors.'],
                ['/directory/holistic-financial-planning.png', 'Holistic Retirement Planning', 'Your Social Security strategy doesn\u2019t exist in isolation. An ARPI advisor integrates it with your full retirement plan so your assets work together toward a secure future.'],
                ['/directory/social-security-guidance.png', 'Personalized Guidance You Can\u2019t Get Elsewhere', 'The Social Security Administration provides information — not strategy. An ARPI-certified advisor delivers tailored solutions based on your unique circumstances and goals.'],
              ].map(([icon, title, body]) => (
                <div key={title} style={{ background: '#fff', padding: '2.5rem 2rem' }}>
                  <div style={{ width: 40, height: 3, background: '#139a4d', marginBottom: 24 }} />
                  <div style={{ width: 52, height: 52, background: '#f0faf4', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
                    <img src={icon} alt="" width="28" height="28" loading="lazy" style={{ width: 28, height: 28, objectFit: 'contain' }} />
                  </div>
                  <h3 style={{ fontFamily: 'var(--font-merriweather), Georgia, serif', fontSize: '1.1rem', fontWeight: 700, color: '#111827', marginBottom: 12, lineHeight: 1.35 }}>{title}</h3>
                  <p style={{ fontSize: '0.9rem', color: '#374151', lineHeight: 1.75 }}>{body}</p>
                </div>
              ))}
            </div>
          </div>
